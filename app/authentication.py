...
def create_token(user):
    try:
        ...
        refresh_payload = {"exp": datetime.UTC() + timedelta(days=app.config["REFRESH_TOKEN_AGE"])}
        
        access_token = jwt.encode(access_payload, app.config["SECRET_KEY"], algorithm="HS256")
        refresh_token = jwt.encode(refresh_payload, app.conf["SECRET_KEY"], algorithm="HS256")
